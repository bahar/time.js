...
The test suite runs successfully on IE6, IE7, Safari 4 Beta (mac), Firefox 3.5 (mac) and Opera 9 (mac).

Coming soon in a version near you: <code>advance({monts: 5})</code>, <code>advance({days: -8})</code> and <code>beginningOfWeek()</code>.
...
<pre><code>new Time(2008, 5, 17).beginningOfYear();              // January 1th 2008, 00:00
...
Useful when rendering month calendar grids. Set the first day of the month with <code>Time.firstDayOfMonth = 1</code>. It defaults to <code>0</code>, which is sunday.
